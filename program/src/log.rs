...
pub extern fn decode_ray_log(log: &str) {
    ...
    // let log: SwapBaseInLog = bincode::deserialize(&bytes).unwrap();
    // // println!("{:?}", log.amount_in);
    // return log.amount_in
}

pub extern fn decode_ray_log_swap_amount_in(log: &str) -> u64 {
    let bytes = base64::decode_config(log, base64::STANDARD).unwrap();
    let log: SwapBaseInLog = bincode::deserialize(&bytes).unwrap();
    return log.amount_in;
}

pub extern fn decode_ray_log_swap_amount_out(log: &str) -> u64 {
    let bytes = base64::decode_config(log, base64::STANDARD).unwrap();
    let log: SwapBaseInLog = bincode::deserialize(&bytes).unwrap();
    return log.out_amount;
}

pub extern fn decode_ray_log_swap_direction(log: &str) -> u64 {
    let bytes = base64::decode_config(log, base64::STANDARD).unwrap();
    let log: SwapBaseInLog = bincode::deserialize(&bytes).unwrap();
    return log.direction;
}

pub extern fn decode_ray_log_init_pc_amount(log: &str) -> u64 {
    let bytes = base64::decode_config(log, base64::STANDARD).unwrap();
    let log: InitLog = bincode::deserialize(&bytes).unwrap();
    return log.pc_amount;
}

pub extern fn decode_ray_log_init_coin_amount(log: &str) -> u64 {
    let bytes = base64::decode_config(log, base64::STANDARD).unwrap();
    let log: InitLog = bincode::deserialize(&bytes).unwrap();
    return log.coin_amount;
}

pub extern fn decode_ray_log_init_coin_decimals(log: &str) -> u8 {
    let bytes = base64::decode_config(log, base64::STANDARD).unwrap();
    let log: InitLog = bincode::deserialize(&bytes).unwrap();
    return log.coin_decimals;
}